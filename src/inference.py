...
with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    ffmpeg["-i", str(INPUT_VIDEO_PATH.absolute()), f"{frames_dir / "out-%06d.jpg"}"]()
    ...
    frames_files = sorted(frames_dir.glob("out-*.jpg"))
    ...
    for idx, frame in enumerate(frames_files, start=1):
        ...
        output_image = draw_bounding_boxes(to_tensor(image), boxes=torch.tensor(preds[0]["boxes"], dtype=torch.float32), colors="red", width=2)
        save_image(output_image, detection_frames_dir / f"det-{frame.name.split('-')[1]}")
        toc = time.perf_counter()
        logger.info(f"[{idx:04d}/{len(frames_files):04d}] Procesato il frame {frame.name} in {toc - tic:.3f} secondi.")
    ...
    ffmpeg["-framerate", "30", "-i", str(detection_frames_dir / "det-%06d.jpg"), "-c:v", "libx264", "-pix_fmt", "yuv420p", str(OUTPUT_VIDEO_PATH.absolute())]()
